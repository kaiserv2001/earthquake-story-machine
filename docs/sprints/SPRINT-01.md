...
- **Deviations (2026-06-10, backend, Wave 3):** (a) #16 plan snippet missing `using Microsoft.Azure.Functions.Worker;` for ConfigureFunctionsApplicationInsights; (b) #16 Unsplash Client-ID DI registration guarded so a placeholder key can't crash host boot; (c) #17 EF Core 8 lacks `ToHashSetAsync` (EF9 API) — `ToListAsync().ToHashSet()` used, behavior identical. Host boot verified with func 4.12.0: all four functions index (2 HTTP, 1 serviceBusTrigger, 1 timerTrigger); `func start` warns isolated projects prefer `dotnet run` (both work).
